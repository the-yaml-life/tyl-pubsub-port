...
use tyl_pubsub_port::{pact_event, EventPublisher, MockPubSubAdapter, ValidatedMockAdapter};
    ...
    async fn test_pact_events_have_schema_methods() {
        ...
        #[cfg(feature = "pact-validation")]
        {
            ...
            assert!(
                ...
                "Example should pass its own schema validation"
            );
        }
...
mod schema_enforcement_tests {
    ...
    async fn test_demonstrate_schema_enforcement() {
        ...
        match result {
            Ok(event_id) => {
                println!(
                    "✅ SUCCESS: Schema validation passed, event published with ID: {}",
                    event_id
                );
