...
mod microservice_usage_tests {
    ...
    async fn test_microservice_without_pact_validation_succeeds() {
        ...
        // Both should succeed
        assert!(order_result.is_ok(), "Order publishing should succeed without validation");
        assert!(user_result.is_ok(), "User publishing should succeed without validation");
    ...
    async fn test_microservice_using_basic_traits_only() {
        ...
        let result2 = pubsub.publish("topic2", serde_json::json!({
            "id": "event-1",
            "data": "some data"
        })).await;
    ...
    async fn test_with_validation_fails_without_consumers() {
        ...
        assert!(result.is_err(), "Validated publishing should fail without consumers");
...
mod integration_simulation {
    ...
    async fn test_gradual_validation_adoption_scenario() {
        ...
        #[cfg(feature = "pact-validation")]
        {
            ...
            validated_pubsub.simulate_consumer_registration("ecommerce.order.created.v1", "payment-service");
            validated_pubsub.simulate_consumer_registration("ecommerce.order.created.v1", "inventory-service");
