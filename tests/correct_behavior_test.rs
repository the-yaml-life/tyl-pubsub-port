...
mod correct_behavior_tests {
    ...
    async fn test_microservice_publishing_fails_without_consumers() {
        ...
        assert!(
            result.is_err(),
            "❌ EXPECTED: Microservice test should fail without consumers"
        );
    ...
    async fn test_microservice_publishing_succeeds_with_consumers() {
        ...
        assert!(
            result.is_ok(),
            "✅ EXPECTED: Should succeed with registered consumers"
        );
    ...
    async fn test_port_basic_functionality_always_works() {
        ...
        assert!(
            result.is_ok(),
            "✅ Port basic functionality should always work"
        );
    ...
    async fn test_different_event_types_require_separate_consumers() {
        ...
        assert!(
            order_result.is_ok(),
            "Order should succeed with registered consumer"
        );
        ...
        assert!(
            user_result.is_err(),
            "User should fail without registered consumer"
        );
    ...
    async fn test_without_pact_feature_everything_works() {
        ...
        assert!(
            result.is_ok(),
            "Should work without pact-validation feature"
        );
...
mod migration_scenario_tests {
    ...
    async fn test_brownfield_migration_scenario() {
        ...
        new_adapter
            .simulate_consumer_registration("orders.order.placed.v1", "notification-service");
