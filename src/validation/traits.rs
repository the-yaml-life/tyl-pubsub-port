...
pub trait PactValidated: Serialize + JsonSchema + Clone + Send + Sync {
    ...
    fn validate_schema(&self) -> PubSubResult<()> {
        ...
        let schema_value = serde_json::to_value(&schema.schema)
            .map_err(|e| ValidationError::SchemaCompilationFailed(format!("Failed to serialize schema: {e}")))?;
        ...
        match result {
            ...
            Err(errors) => {
                let error_messages: Vec<String> = errors
                    .map(|e| e.to_string())
                    .collect();
                Err(ValidationError::SchemaValidationFailed { errors: error_messages }.into())
            }
        }
    }
    ...
    fn generate_consumer_expectation(&self) -> PactMessageExpectation {
        let example_json = serde_json::to_value(self.clone())
    ...
    fn generate_provider_verification(&self) -> PactProviderVerification {
        PactProviderVerification {
            ...
            example: serde_json::to_value(self.clone())
                .expect("Event should be serializable"),
        }
    }
}
